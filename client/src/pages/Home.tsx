...
type KnowledgeSource = { name: string; excerpt: string };
...
export default function Home() {
  ...
  const handleSend = (content: string) => {
    ...
    setMessages(next);
    const queryTerms = content.toLowerCase().split(/\W+/).filter((term) => term.length > 2);
    const relevantSources = sources
      .map((source) => ({ source, score: queryTerms.reduce((score, term) => score + (source.excerpt.toLowerCase().includes(term) ? 1 : 0), 0) }))
  ...
  const handleFile = async (file?: File) => {
    ...
    try {
      ...
      setSources(chunks.slice(0, 8).map((excerpt, index) => ({ name: chunks.length > 1 ? `${file.name} · bagian ${index + 1}` : file.name, excerpt })));
